...
function syncPlainCopySheet(sourceSheet) {
  ...
  const copySheet = recreatePlainCopySheet(sourceSheet, copySheetName);
  removeSheetFilter(copySheet);
  replaceFormulasWithValues(sourceSheet, copySheet);

  Logger.log(`Лист-копия "${copySheetName}" для листа "${sourceSheetName}" обновлён: формулы заменены значениями.`);
}
...
/**
 * Recreates the copy sheet from the source sheet to preserve current formatting quickly.
 *
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sourceSheet Source sheet.
 * @param {string} copySheetName Destination sheet name.
 * @returns {GoogleAppsScript.Spreadsheet.Sheet} Recreated copy sheet.
 */
function recreatePlainCopySheet(sourceSheet, copySheetName) {
  const spreadsheet = sourceSheet.getParent();
  const existingCopySheet = spreadsheet.getSheetByName(copySheetName);
  const targetIndex = existingCopySheet ? existingCopySheet.getIndex() : sourceSheet.getIndex() + 1;
  ...
  copySheet.setIndex(targetIndex);

  Logger.log(`Создан лист-копия "${copySheetName}" для листа "${sourceSheet.getName()}".`);
  return copySheet;
}

/**
 * Removes a filter from the destination sheet.
 *
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sheet Spreadsheet sheet.
 */
...
/**
 * Replaces formulas in the copy with calculated values while restoring source formatting.
 *
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sourceSheet Source sheet.
 * @param {GoogleAppsScript.Spreadsheet.Sheet} copySheet Destination sheet.
 */
function replaceFormulasWithValues(sourceSheet, copySheet) {
